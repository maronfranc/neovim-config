...
function M:get_css_selectors(params)
	...
	local is_after_class_quote = string.match(params.context.cursor_before_line, REGEX_HTML_OPEN_CLASS) ~= nil
	if is_after_class_quote then return self:get_css_classes() end

	local is_after_id_quote = string.match(params.context.cursor_before_line, REGEX_HTML_OPEN_ID) ~= nil
