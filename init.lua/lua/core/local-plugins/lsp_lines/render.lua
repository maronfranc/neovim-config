...
function M.show(namespace, bufnr, diagnostics, opts, source)
	...
	for _, diagnostic in ipairs(diagnostics) do
		if line_stacks[diagnostic.lnum] == nil then line_stacks[diagnostic.lnum] = {} end
			...
			if lelements[i][1] == DIAGNOSTIC then
				...
				for j = 1, i - 1 do
					...
					elseif type == DIAGNOSTIC then
						...
						if lelements[j + 1][1] ~= OVERLAP then table.insert(left, { "│", highlight_groups[data.severity] }) end
						overlap = false
...
---@param namespace number
---@param bufnr number
function M.hide(namespace, bufnr) vim.api.nvim_buf_clear_namespace(bufnr, namespace, 0, -1) end
