...
M.setup = function()
	...
	vim.diagnostic.handlers.virtual_lines = {
		...
		show = function(namespace, bufnr, diagnostics, opts)
			...
			if not ns.user_data.virt_lines_ns then ns.user_data.virt_lines_ns = vim.api.nvim_create_namespace("") end

			vim.api.nvim_clear_autocmds({ group = "LspLines" })
			if opts.virtual_lines.only_current_line then
				vim.api.nvim_create_autocmd("CursorMoved", {
					buffer = bufnr,
					callback = function() render_current_line(diagnostics, ns.user_data.virt_lines_ns, bufnr, opts) end,
					group = "LspLines",
				})
