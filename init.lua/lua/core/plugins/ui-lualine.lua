...
local M = {
	...
	config = function()
		...
		local section_absolute_path = {
			"filename",
			cond = function() return conditions.buffer_not_empty() and conditions.hide_in_width() end,
		...
		local section_filename = {
			...
			fmt = function(path, _) -- (path, context)
				...
				local matched_filename = path:match(REGEX_FILENAME_AND_EXT) or path:match(REGEX_FILENAME) or ""
